...
function notification(message = '', status = 'success') {
  return gulp.src('./node_modules/gulp-notify/test/fixtures/1.txt').pipe(
    notify({
      title: 'Barebones',
      message,
...
gulp.task('clean', () =>
  gulp
    .src([`${config.base.public}/css`, `${config.base.public}/js`], {
      read: false,
    })
    .pipe(clean())
);
...
const roll = (entry, dest) => {
  ...
  return rollup({
    ...
    plugins: [
      ...
      commonjs({
        include: ['node_modules/**', `${config.base.src}/**`],
      }),
      json(),
      replace({
 ...
 */
gulp.task('images', (cb) => {
  // handle all images that are not svg
  gulp
    .src(`${config.base.src}/images/**/*.*`)
    .pipe(
      imagemin({
        progressive: true,
        svgoPlugins: [
    ...
    .pipe(gulp.dest(`${config.base.res}/img`))
    .on('end', () => cb());
});
